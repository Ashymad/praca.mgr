...
\begin{equation}
  \left<x\right> = \left|\log_{q}\left(x_{1}\right)\right| +
  \left|\log_{q}\left(x_{2}\right)\right| + \cdots +
  \left|\log_{q}\left(x_{K}\right)\right|
\end{equation}
...
\begin{equation}
  ...
  \end{cases}.
\end{equation}
Z czego wyprowadzono następujący algorytm:
\begin{equation}
  \begin{cases}
    \overline{x_1} &= x_1\\
    \overline{x_{k+1}} &= \begin{cases}
      \overline{x_k}x_{k+1}, & (\overline{x_k} \geq 1 \land x_{k+1} \geq 1) \lor
      (\overline{x_k} < 1 \land x_{k+1} < 1)\\
      \frac{\overline{x_k}}{x_{k+1}}, & (\overline{x_k} > 1 \land x_{k+1} < 1)
      \lor (\overline{x_k} < 1 \land x_{k+1} > 1)\\
  \end{cases}
  \end{cases},
\end{equation}
\begin{equation}
  \left<x\right> = |\log_q(\overline{x_K})|.
\end{equation}
\begin{figure}[!tbh]
  \centering
