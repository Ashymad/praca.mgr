...
\@ifclassloaded{aghdpl}{%
  ...
  \summaryPL{\indent\indent%
    Obecnie wszechobecnie stosowane są formaty kompresji audio, w celu
    zmniejszenia rozmiaru plików, a więc ograniczenia miejsca na dysku i
    ograniczenia przesyłu danych przez internet. Kompresja stratna znacząco
    zmniejsza ten rozmiar, poświęcając możliwość odtworzenia oryginalnego
    pliku. W obrocie mogą się znajdować pliki uznane za nieskompresowane, które
    uzyskano drogą dekompresji z pliku w formacie skompresowanym stratnie.
    W pracy przedstawiono rozwiązanie, pozwalające na rozpoznanie czy dany
    fragment sygnału audio został poddany kompresji stratnej. Wykorzystano w
    tym celu techniki uczenia maszynowego, w szczególności, skonstruowano sieć
    opartą o Konwolucyjne Sieci Neuronowe, rozpoznającą na podstawie
    spektrogramów. Sieć wytrenowano na zgromadzonej bazie fragmentów
    muzyki, uzyskując dokładność \SI{97.4}{\percent}. Rozszerzono także
    zagadnienie o rozpoznanie typu i stopnia kompresji zastosowanego do pliku, z
    równie dobrymi wynikami, uzyskując dokładność \SI{92.4}{\percent}.
    Podjęto się także implementacji algorytmu niewymagającego uczenia opartego
    o wykrywanie dużych różnic, pomiędzy kolejnymi średnimi energiami wyników
    transformaty MDCT. Algorytm usprawniono poprzez dodanie kroków rafinujących
    wyniki oraz wykorzystanie efektywnych procedur. W szczególności,
    przedstawiono algorytm obliczający średnią modułów logarytmów modułów,
    oferujący czas wykonania ponad dwa i pół raza krótszy niż rozwiązanie
    alternatywne.
  }
  \summaryEN{\indent\indent%
    Currently, audio file compression is used everywhere, to reduce the size of
    files, which subsequently means reduction of disk usage as well as
    reduction of data sent through the web. Lossy compression reduces the size
    extremely effectively, but gives up the possiblity to recreate the original
    file. Files, thought to be uncompressed, but obtained through decompression
    of a lossy compressed file, can be sold and bought. The thesis presents a
    software solution which can be used to detect if an audio signal fragment
    was previously lossy compressed. To achieve that, machine learning
    techniques were employed, a Convolutional Neural Network was architectured,
    that learns on spectrograms. The network was trained on a collected music
    fragment database, reaching accuracy of \SI{97.4}{\percent}. The problem
    was also extended to detection of the type and the bitrate of the used
    codec, reaching similarly good accuracy of \SI{92.4}{\percent}. A
    algorithm without a need to be trained, based on detection of big
    differences of subsequent mean energies of the MDCT transform, was
    implemented as well. The algorithm was improved, by adding steps refining
    the results, and optimized. A algorithm to calculate the mean of absolute
    values of logarithms of absolute values was explained, that is over two and
    a half times faster the a naive version.
  }
...
\begin{equation}
  ...
  \end{bmatrix}
\end{equation}
Trzecim typem warstw, stosowanych w sieciach CNN, są warstwy w pełni połączone
(eng. \textit{FC -- Fully Connected})~\cite{RamsundarZadeh2018}. Tego typu warstwy umieszcza się na
