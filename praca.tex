...
2c$, gdzie $c \in \bm N$, oraz
skok $h \in \bm N$. Długość okna określa długość fragmentu sygnału $s$ który zostanie pomnożony
przez okno $w$, a następnie poddany transformacji, natomiast skok określa co ile
próbek jest wycinany kolejny fragment. Wynikiem całej operacji jest jest
macierz $\bm X(f) = \begin{bmatrix}X_1(f) & X_2(f) & \cdots  &
X_k(f)\end{bmatrix}$, której elementy to wektory 
\[X_n =
\text{MDCT}\{w\begin{bmatrix}s_{(n-1)h}\\\vdots\\s_{(n-1)h+l}\end{bmatrix}\},\]
gdzie okno $w$ jest wektorem o długości $l$, wyznaczonym na podstawie pewnej
funkcji, której zadaniem jest zniwelowanie efektów brzegowych. Aby
transformacja była odwracalna, okno musi być symetryczne, tzn.\ spełniać
warunek $w_n = w_{l-1-n}$, oraz musi spełniać warunek Princena-Bradleya
...
następnie rozszerzony w standardach MPEG-2 oraz MPEG-2.5.
...
W procesie MDCT zastosowane jest okno sinusowe (nazywane też kosinusowym) (rys.
\ref{fig:win_MP3}) o następującym wzorze:
\begin{equation}
  w(n) = \sin\left(\frac{\pi n}{N}\right) = \cos\left(\frac{\pi n}{N} -
  \frac{\pi}{2}\right).
\end{equation}
